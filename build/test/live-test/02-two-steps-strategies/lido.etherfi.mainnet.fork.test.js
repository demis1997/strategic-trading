...
let etherFiAdapterContractAddress;
...
describe("LIDO - ETHERFI ==> Restaking Strategy Tests -->> LIVE TESTS", function () {
    ...
    before(async () => {
        ...
        liquidTokenContract = await (0, emulation_1.getEethContract)(constants_1.ETHERFI_eETH_ADDRESS, deployer);
        ...
        etherFiAdapterContract = (await (0, _deployContracts_1.deployEtherFiAdapter)("EtherFiAdapter", constants_1.ETHERFI_LIQUIFIER_ADDRESS, constants_1.ETHERFI_eETH_ADDRESS, constants_1.ETHERFI_weETH_ADDRESS));
        ...
        strategyContract = (await (0, _deployContracts_1.deployVaultStrategy)("StrSimpleReStaking", vaultAddress, constants_1.ETHERFI_eETH_ADDRESS, [lidoAdapterContractAddress, etherFiAdapterContractAddress], [uniswapAdapterContractAddress], "Simple Re-Staking Strategy"));
        ...
        await (0, _configContracts_1.configDepositStrategy)("StrSimpleReStaking", strategyContractAddress, constants_1.ZERO_ADDRESS, constants_1.ETHERFI_weETH_ADDRESS);
        await (0, _configContracts_1.configSwapPath)("StrSimpleReStaking", strategyContractAddress, [constants_1.WETH_ADDRESS, constants_1.ETHERFI_weETH_ADDRESS], [UNISWAP_PATH_FEE]);
        oracleContract = (await (0, _deployContracts_1.deployOracleMock)("OracleMock", constants_1.ETHERFI_eETH_ADDRESS, "eeETH", BigInt("998700000000000000")));
        oracleContractAddress = await oracleContract.getAddress();
        await strategyContract.setPriceFeedPerToken(constants_1.ETHERFI_eETH_ADDRESS, oracleContractAddress);
        await strategyContract.setPriceFeedPerToken(constants_1.ETHERFI_weETH_ADDRESS, constants_1.WeETH_ETH_feed);
        ...
        deltaEtherFi = await (0, utils_1.getDelta)(constants_1.LIDO_stETH_ADDRESS, constants_1.ETHERFI_eETH_ADDRESS, deployer, deltaLido.outAmount, etherFiAdapterContract);
        const AMOUNT_OUT = constants_1.AMOUNT_1E18 * 5n;
        uniQuote = await (0, utils_1.getQuoteFromUni)("KELP", uniswapAdapterContract, AMOUNT_OUT, constants_1.ETHERFI_weETH_ADDRESS, constants_1.WETH_ADDRESS, UNISWAP_PATH_FEE);
        oneEethInWETH = await strategyContract.getTokenPrice(constants_1.ETHERFI_eETH_ADDRESS, 2);
        expectedBalanceAferDeployment = (deltaLido.outAmount * constants_1.AMOUNT_1E18) / deltaEtherFi.delta;
        expectedInitialValuation = (expectedBalanceAferDeployment * oneEethInWETH) / constants_1.AMOUNT_1E18;
    });
    ...
    describe("WHEN executing withdraw by user1", function () {
        ...
        it("THEN strategy balance should decrease", async () => {
            oneEethInWETH = await strategyContract.getTokenPrice(constants_1.ETHERFI_eETH_ADDRESS, 2);
            const oneWeethInWETH = await strategyContract.getTokenPrice(constants_1.ETHERFI_weETH_ADDRESS, 2);
            const qtyWrapped = uniQuote;
            const qtyUnWrapped = (oneWeethInWETH * qtyWrapped) / oneEethInWETH;
            const tolerance = 4000000000000000n;
        ...
        describe("WHEN executing update valuation from vault (C)", function () {
            ...
            it("THEN vault valuation should decrease close to 15 WETH", async () => {
                oneEethInWETH = await strategyContract.getTokenPrice(constants_1.ETHERFI_eETH_ADDRESS, 2);
                const newValuation = (strategyStokenBalance * oneEethInWETH) / constants_1.AMOUNT_1E18;
                const vaultValuation = await vaultContract.totalAssets();
                const tolerance = 1n;
...
//# sourceMappingURL=lido.etherfi.mainnet.fork.test.js.map