...
let staderAdapterContract: TestnetAdapterMock;
let staderAdapterContractAddress: string;
...
describe("STADER - KELP ==> Restaking Strategy Tests -->> MOCK TESTS", function () {
    ...
    before(async () => {
        ...
        // deploy weth-stader adapter for staking protocol
        const MockAdapterFactory = await ethers.getContractFactory("TestnetAdapterMock");
        staderAdapterContract = (await MockAdapterFactory.deploy(
            "weth-stader-weth",
            underlyingTokenAddress,
        ...
        tx = await firstLiquidTokenContract.mint(staderAdapterContractAddress, INTIAL_MINT * 100n);
        await tx.wait();
        ...
        strategyContract = (await deployVaultStrategy(
            ...
            vaultAddress,
            liquidTokenContractAddress,
            [staderAdapterContractAddress, kelpAdapterContractAddress],
        ...
        percentageDeposit.push(await staderAdapterContract.percentageModifierDeposit());
        percentageWithdraw.push(await staderAdapterContract.percentageModifierWithdraw());
