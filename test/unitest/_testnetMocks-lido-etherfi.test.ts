...
let etherfiAdapterContract: TestnetAdapterMock;
...
const LIQUID_TKN_IN_WETH = 999500000000000000n; // eETH

describe("LIDO - ETHERFI ==> Restaking Strategy Tests -->> MOCK TESTS", function () {
    ...
    before(async () => {
        ...
        // deploy lido-etherfi
        etherfiAdapterContract = (await MockAdapterFactory.deploy(
            "lido-etherfi-weth",
            firstLiquidTokenContractAddress,
        ...
        tx = await wethTokenContract.mint(etherfiAdapterContractAddress, INTIAL_MINT * 100n);
        await tx.wait();
        ...
        tx = await liquidTokenContract.mint(etherfiAdapterContractAddress, INTIAL_MINT * 100n);
        await tx.wait();
        ...
        await lidoAdapterContract.setPercentageDeposit(0);
        await etherfiAdapterContract.setPercentageDeposit(0);
        await etherfiAdapterContract.setPercentageDeposit(4);
        ...
        percentageDeposit.push(await etherfiAdapterContract.percentageModifierDeposit());
        percentageWithdraw.push(await etherfiAdapterContract.percentageModifierWithdraw());
