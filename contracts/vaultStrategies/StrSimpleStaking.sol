...
/// @notice Implements simple staking strategy for a single asset using adapters
contract StrSimpleStaking is BaseStrategy, IDeployStrategy {
    /// @notice Number of deployment adapters specified for this strategy
    uint8 public constant DEPLOYMENT_ADAPTERS_QTY = 1;
    ...
    function executeDeploymentStrategy(
    ...
    ) external onlyRole(VAULT_MANAGER_ROLE) returns (address, uint256) {
        ...
        // check result
        if (liquidTokenAmount == 0 || liquidToken != liquidTokenAddress) {
        ...
        // emit event
        emit DeploymentStrategyExecuted(asset_, assetsAmount_, liquidToken, liquidTokenAmount);
    ...
    function updateDeployedAssetVaule(
        ...
        uint256 oneTokenInEth = getTokenPrice(liquidTokenAddress, 2);
